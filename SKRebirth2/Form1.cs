    ...
    public partial class Form1 : MaterialForm
    {
        ...
        private void materialButton2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                this.stucchiDataSet.AcceptChanges();
        ...
        private void materialButton11_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string cnpj_del = (string)currentRow["CNPJ"];
                ...
                using (MySqlConnection con = new MySqlConnection(connection))
                {
                    ...
                    {
                        cmd.Parameters.AddWithValue("@CNPJ", cnpj_del);
                        cmd.ExecuteNonQuery();
                    }
                }
                empresa_nacionalBindingSource.RemoveCurrent();
                stucchiDataSet.AcceptChanges();
        ...
        private void materialButton6_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        mail.From = new MailAddress("stucchisistemas@gmail.com", "Source Kode");
        ...
        private void materialButton14_Click(object sender, EventArgs e)
        {
            try 
            {
                DataRowView currentRow = (DataRowView)funcionariosBindingSource.Current;
                string cpf_del = (string)currentRow["cpf"];
                string connection = "server=localhost;user id=root;password=;database=stucchi";
                using (MySqlConnection con = new MySqlConnection(connection))
                {
                    con.Open();
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, con))
                    {
                        cmd.Parameters.AddWithValue("@cpf", cpf_del);
                ...
                MaterialMessageBox.Show("Registro deletado com sucesso." + cpf_del);
            }
            catch (Exception ex)
            {
                MaterialMessageBox.Show("Erro ao deletar registro: " + ex.Message);
        ...
        {
            {
                try
                {
                    ...
                    using (MySqlConnection con = new MySqlConnection(connection))
                    {
                        ...
                        this.empresa_internacionalTableAdapter.Fill(this.stucchiDataSet.empresa_internacional); 
                        empresa_internacionalBindingSource.DataSource = this.stucchiDataSet.empresa_internacional; 
                        empresa_internacionalBindingSource.ResetBindings(false);
                        empresa_internacionalDataGridView.DataSource = empresa_internacionalBindingSource;

                        this.funcionariosTableAdapter.Fill(this.stucchiDataSet.funcionarios);
                        funcionariosBindingSource.DataSource = this.stucchiDataSet.funcionarios;
                        funcionariosBindingSource.ResetBindings(false);
                        funcionariosDataGridView.DataSource = funcionariosBindingSource;
        ...
        private void materialButton13_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string ruc_del = (string)currentRow["ruc"];
                string rut_del = (string)currentRow["rut"];
                string connection = "server=localhost;user id=root;password=;database=stucchi";
                using (MySqlConnection con = new MySqlConnection(connection))
                {
                    con.Open();
                    string query = "DELETE FROM empresa_internacional WHERE ruc = @ruc  AND rut = @rut";
                    using (MySqlCommand cmd = new MySqlCommand(query, con))
                    {
                        cmd.Parameters.AddWithValue("@ruc", ruc_del);
                        cmd.Parameters.AddWithValue("@rut", rut_del);
                        cmd.ExecuteNonQueryAsync();
                        cmd.CommandTimeout = 200;
                    }
                }
                ...
                stucchiDataSet.AcceptChanges();
                this.empresa_internacionalTableAdapter.Fill(stucchiDataSet.empresa_internacional);
